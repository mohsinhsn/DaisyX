...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            ...
            return
        await lel.edit(
            f"Tanya AI Successfully Added For Users In The Chat {message.chat.id}"
        )
        ...
        if not Escobar:
            ...
            return
        await lel.edit(
            f"Tanya AI Successfully Deactivated For Users In The Chat {message.chat.id}"
        )
...
async def inuka(client, message):
    ...
    # Kang with the credits bitches @monstar_0
    test = test.replace("Tanya", "Aco")
    test = test.replace("Tanya", "Aco")
...
@daisyx.on_message(
    filters.regex("Tanya|tanya|TANYA")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
...
__help__ = """
<b> Chatbot </b>
TANYA AI IS THE ONLY AI SYSTEM WHICH CAN DETECT & REPLY UPTO 200 LANGUAGES
...
<b> Assistant </b>
 - /ask [question]: Ask question from Tanya
